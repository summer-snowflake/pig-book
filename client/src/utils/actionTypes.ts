export const LOGIN_REQUEST = 'LOGIN_REQUEST';
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS';
export const LOGIN_FAILURE = 'LOGIN_FAILURE';

export const LOGOUT_SUCCESS = 'LOGOUT_SUCCESS';

export const GET_USER_STATUS_REQUEST = 'GET_USER_STATUS_REQUEST';
export const GET_USER_STATUS_SUCCESS = 'GET_USER_STATUS_SUCCESS';
export const GET_USER_STATUS_FAILURE = 'GET_USER_STATUS_FAILURE';

export const GET_SETTINGS_REQUEST = 'GET_SETTINGS_REQUEST';
export const GET_SETTINGS_SUCCESS = 'GET_SETTINGS_SUCCESS';
export const GET_SETTINGS_FAILURE = 'GET_SETTINGS_FAILURE';

export const CHANGE_SETTINGS_LOCALE = 'CHANGE_SETTINGS_LOCALE';
export const CHANGE_SETTINGS_CURRENCY = 'CHANGE_SETTINGS_CURRENCY';
export const SET_EDITING = 'SET_EDITING';

export const PATCH_SETTINGS_REQUEST = 'PATCH_SETTINGS_REQUEST';
export const PATCH_SETTINGS_SUCCESS = 'PATCH_SETTINGS_SUCCESS';
export const PATCH_SETTINGS_FAILURE = 'PATCH_SETTINGS_FAILURE';

