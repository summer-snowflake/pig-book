...
describe 'POST /api/import_histories/:import_history_id/create_category' do
  let!(:user) { create(:user) }
  let!(:import_history) { create(:import_history, user: user) }

  context 'ログインしていなかった場合' do
    it '401とデータが返ってくること' do
      post "/api/import_histories/#{import_history.id}/create_category"
  ...
  context 'ログインしていた場合' do
    it '201が返ってくること' do
      ...
      expect(category.name).to eq '医療費'
    end
  end
end
